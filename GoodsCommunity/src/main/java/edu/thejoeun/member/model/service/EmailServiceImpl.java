...
@Service
@Transactional // 예외 발생 시 롤백할게 (기본값으로 커밋)
public class EmailServiceImpl implements EmailService {

    @Autowired // EmailConfig 에 설정된 메일 보내기 기능과 관련 환경설정 사용
    ...
    public String sendMail(String htmlName, String email) {
        ...
        try {
            ...
            // 인증 메일 보내기
            // MimeMessage : Java 에서 메일을 보내기 위해 메세지를 준비하는 객체
            MimeMessage mimeMessage = mailSender.createMimeMessage();
            //                                              메세지내용      파일 전송 사용?    문자 인코딩 지정
            // mailSender 객체를 이용해서 이메일 본문을 쉽게 설정할 수 있도록 도와줌
            MimeMessageHelper helper = new MimeMessageHelper(mimeMessage, true, "UTF-8");
            helper.setTo(email);    // 이메일 받을 사람 지정
            helper.setSubject(제목); // 이메일 제목 지정
            helper.setText(loadHtml(authKey, htmlName), true); // 이메일 본문에는 인증키 + 회사에서 인증키를 보냈다는 신뢰의 본문 내용
                                                                    // true : html 형식이 맞음 (false 를 쓰는 경우 추가 설정 필요)
            // 메일에 이미지 첨부. 파일 첨부랑은 살짝 다름.
            //  로고 이미지를 보낼 때, 보낼 이미지와 이미지를 보낼 때 담을 id 명칭 작성
        ...
        // 형태로 잠시 Java 에서 보관하고 있음.
        Map<String, String> map = new HashMap<>();
        map.put("authKey", authKey);
        map.put("email", email);

        // 이전에 해당 이메일로 인증키를 발송한 기록이 있다면 키를 갱신
        int result = emailMapper.updateAuthKey(map); // 발송한 기록이 없으면 0이 result 에 담길 것

        // 이전에 인증키를 보낸 적이 없으면
        if(result == 0) {
            result = emailMapper.insertAUthKey(map); // 새로운 인증키 기록을 삽입하는 mapper 메서드 호출
            // 이메일에서 인증키를 무사히 보냈다면 result 에는 1이 담길 것
        }

        // 둘 다 실패하여 최종 result 가 0이라면 null 반환
        if(result ==0){
            return null;
        }
        ...
        매개변수에 들어갈 데이터는            String, String 형태로,
        매개변수 내부 자료형 데이터와 매개변수 내부에 들어갈 데이터 자료형이 일치하지 않아 발생하는 빨간줄!
         */
    ...
    public String createAuthKey(){
        ...
        for(int i=0; i<6; i++){ // key 의 내부가 6자리로 채워지기 위해 0 ~ 5 까지 5번 반복
             ...
             *  key += ch;
             *
             * 영어 중에서 대문자만 랜덤으로 6자리 출력하겠다.
             *  char ch = (char)(Math.random()*26 + 65);
             *  key += ch;
             *
             * 영어 중에서 소문자만 랜덤으로 6자리 출력하겠다.
             *  char ch = (char)(Math.random()*26 + 65);
             *  ch = (char)(ch + ('a' - 'A'));
             *  key += ch;
             **************/

            // 0,1,2 중 하나의 무작위 정수 뽑기. 뽑힌 숫자는 sel1 안에 저장
            // 만약에 0 이 뽑히면 숫자   1,2 가 뽑히면 영어(대문자 or 소문자)
            int sel1 = (int)(Math.random() * 3); // 0 : 숫자 / 1, 2 영어

            if(sel1==0) { // sel1 에 뽑힌 숫자가 0이면 숫자 당첨!
                int num = (int)(Math.random() * 10); // 0 ~ 9
                key += num; // key 내부에 랜덤으로 뽑힌 숫자 하나가 들어감.
            } else { // 0 이 아니면 일단 영어(대문자 or 소문자)이기 때문에 else if 가 아니라 else 로 처리

                // 우선은 무작위로 A부터 Z까지 대문자를 생성해서 ch 에 저장.
                char ch = (char)(Math.random()*26 + 65); // A ~ Z (A = 65) => 65~(65+26)

                // 또 랜덤 뽑기를 해서 0이 나오면 소문자, 1이 나오면 대문자.
                int sel2 = (int)(Math.random() * 2); // 0 : 소문자, 1 : 대문자
                if(sel2==0) {
                    ch = (char)(ch + ('a' - 'A')); // 대문자를 소문자로 변경
                }

                key += ch; // 위에서 랜덤으로 생성된 소문자나 대문자를 key 내부에 추가.
            }
        }
        return key;
    }
