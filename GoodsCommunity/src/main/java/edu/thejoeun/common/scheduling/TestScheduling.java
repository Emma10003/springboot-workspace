 ...
 *
 *  * 주의사항 *
 *  - 해당 메서드는 반환형이 존재해서는 안 된다.
 */
@Component  // -> Bean 으로 등록 => Spring 이 자동으로 스케쥴링 코드를 수행하게 함.
@Slf4j      // -> log를 작성할 수 있는 어노테이션
public class TestScheduling {

    // @Scheduled // 매개변수
    // fixedDelay :
    // 이전 작업이 끝난 후 다음 작업이 시작될 때 까지의 시간을 지정

    // fixedRate :
    // 이전 작업이 시작한 후 다음 작업이 시작될 때 까지의 시간을 지정
    ...
    // @Scheduled(cron="0 59 * * * *")  // 1분마다 수행 -> 분을 적을 때 초는 무조건 있어야 함.
    // @Scheduled(cron="0/10 * * * * *")  // 0초부터 10초마다 수행 (1분마다 수행)
    // @Scheduled(cron="0 0 0 * * *")  // 매일 자정마다 수행
    // @Scheduled(cron="59 59 23 * * *")  // 다음 날 되기 1초 전에 수행
    public void testMethod(){
        log.info("0초 스케줄러 테스트 중입니다...");
    }
}
